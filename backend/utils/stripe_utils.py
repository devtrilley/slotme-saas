...
def cancel_subscription(subscription_id: str) -> stripe.Subscription:
    ...
    try:
        ...
        if subscription.status in ["canceled", "incomplete_expired"]:
            print(f"ℹ️  Subscription {subscription_id} already in terminal state: {subscription.status}")
            return subscription
        ...
        print(f"✅ Successfully scheduled cancellation for subscription {subscription_id} at period end")
        
    except stripe.error.InvalidRequestError as e:
        # Handle "no such subscription" gracefully
        if "No such subscription" in str(e):
            print(f"ℹ️  Subscription {subscription_id} not found (may have been deleted)")
            return None
        raise
...
def cancel_freelancer_subscription(stripe_customer_id):
    ...
    try:
        ...
        for subscription in subscriptions.data:
            ...
            print(f"✅ Scheduled cancellation for subscription {subscription.id} at period end")
            canceled_count += 1
